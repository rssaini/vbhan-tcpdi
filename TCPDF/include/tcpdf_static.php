...
class TCPDF_STATIC {
	...
	public static function getObjFilename($type='tmp', $file_id='') {
		return tempnam(VBHAN_K_PATH_CACHE, '__tcpdf_'.$file_id.'_'.$type.'_'.md5(TCPDF_STATIC::getRandomSeed()).'_');
	}
	...
	public static function getAnnotOptFromJSProp($prop, &$spot_colors, $rtl=false) {
		...
		if (isset($prop['fillColor'])) {
			if (is_array($prop['fillColor'])) {
				...
				$opt['mk']['bg'] = VBHAN\TCPDF_COLORS::convertHTMLColorToDec($prop['fillColor'], $spot_colors);
			}
		}
		...
		if (isset($prop['strokeColor'])) {
			if (is_array($prop['strokeColor'])) {
				...
				$opt['mk']['bc'] = VBHAN\TCPDF_COLORS::convertHTMLColorToDec($prop['strokeColor'], $spot_colors);
			}
		}
