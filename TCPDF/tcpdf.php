...
class TCPDF {
	 ...
	 */
	protected $cell_height_ratio = VBHAN_K_CELL_HEIGHT_RATIO;
	...
	public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=false) {
		...
		$this->InFooter = false;
		$this->lasth = 0;
		$this->FontFamily = defined('VBHAN_PDF_FONT_NAME_MAIN')?VBHAN_PDF_FONT_NAME_MAIN:'helvetica';
		$this->FontStyle = '';
		$this->FontSizePt = 12;
	...
	public function Error($msg) {
		...
		$this->_destroy(true);
		if (defined('VBHAN_K_TCPDF_THROW_EXCEPTION_ERROR') AND !VBHAN_K_TCPDF_THROW_EXCEPTION_ERROR) {
	...
	public function Close() {
		...
		if ($this->tcpdflink) {
			...
			$font = defined('VBHAN_PDF_FONT_NAME_MAIN')?VBHAN_PDF_FONT_NAME_MAIN:'helvetica';
			$this->SetFont($font, '', 1);
	...
	public function Header() {
		if ($this->header_xobjid === false) {
			...
			if (($headerdata['logo']) AND ($headerdata['logo'] != VBHAN_K_BLANK_IMAGE)) {
				$imgtype = TCPDF_IMAGES::getImageFileType(VBHAN_K_PATH_IMAGES.$headerdata['logo']);
				if (($imgtype == 'eps') OR ($imgtype == 'ai')) {
					$this->ImageEps(VBHAN_K_PATH_IMAGES.$headerdata['logo'], '', '', $headerdata['logo_width']);
				} elseif ($imgtype == 'svg') {
					$this->ImageSVG(VBHAN_K_PATH_IMAGES.$headerdata['logo'], '', '', $headerdata['logo_width']);
				} else {
					$this->Image(VBHAN_K_PATH_IMAGES.$headerdata['logo'], '', '', $headerdata['logo_width']);
				}
	 ...
	 * The definition file (and the font file itself when embedding) must be present either in the current directory or in the one indicated by VBHAN_K_PATH_FONTS if the constant is defined. If it could not be found, the error "Could not include font definition file" is generated.
	 ...
	 * Note: for the standard fonts, the font metric files must be accessible. There are three possibilities for this:<ul><li>They are in the current directory (the one where the running script lies)</li><li>They are in one of the directories defined by the include_path parameter</li><li>They are in the directory defined by the VBHAN_K_PATH_FONTS constant</li></ul><br />
		...
		if ($txt != '') {
			...
			if ($this->isunicode) {
				if (($this->CurrentFont['type'] == 'core') OR ($this->CurrentFont['type'] == 'TrueType') OR ($this->CurrentFont['type'] == 'Type1')) {
					...
					if (defined('VBHAN_K_THAI_TOPCHARS') AND (VBHAN_K_THAI_TOPCHARS == true)) {
						// number of chars
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if (in_array($file, $this->imagekeys)) {
			...
			$tempfile_plain = VBHAN_K_PATH_CACHE.'__tcpdf_'.$this->file_id.'_imgmask_plain_'.$filehash;
			// create temp alpha file
			$tempfile_alpha = VBHAN_K_PATH_CACHE.'__tcpdf_'.$this->file_id.'_imgmask_alpha_'.$filehash;
	...
	public function _destroy($destroyall=false, $preserve_objcopy=false) {
		if ($destroyall AND !$preserve_objcopy) {
			...
			$tmpfiles = glob(VBHAN_K_PATH_CACHE.'__tcpdf_'.$this->file_id.'_*');
			if (!empty($tmpfiles)) {
				...
				if ($element[0] == '/') {
					...
					if (($dom[$key]['value'] == 'small') OR ($dom[$key]['value'] == 'sup') OR ($dom[$key]['value'] == 'sub')) {
						if (!isset($dom[$key]['attribute']['size']) AND !isset($dom[$key]['style']['font-size'])) {
							$dom[$key]['fontsize'] = $dom[$key]['fontsize'] * VBHAN_K_SMALL_RATIO;
						}
					}
...
Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
	...
	protected function openHTMLTagHandler($dom, $key, $cell) {
		...
		switch($tag['value']) {
			...
			case 'tcpdf': {
				if (defined('VBHAN_K_TCPDF_CALLS_IN_HTML') AND (VBHAN_K_TCPDF_CALLS_IN_HTML === true)) {
				...
				if (!$clipping) {
					...
					if (preg_match('/^data:image\/[^;]+;base64,/', $img, $m) > 0) {
						...
						if (!isset($testscrtype['query']) OR empty($testscrtype['query'])) {
							...
							$img = str_replace(VBHAN_K_PATH_URL, VBHAN_K_PATH_MAIN, $img);
						}
					}
					// get image type
